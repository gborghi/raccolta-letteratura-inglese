---
title: The Field Bazaar
author: Conan Doyle
type: work
cluster: Detective Fiction · Ratiocination Detection · Crime and Punishment
source: Authors/Conan_Doyle/_raw/THE_FIELD_BAZAAR.md
tags:
  - graph/work
  - author/Conan_Doyle
  - form/short_story
  - form/detective_fiction
  - form/parody
  - concept/ratiocination_detection
  - concept/appearance_vs_reality
  - motif/letters_and_writing
  - setting/london
  - character/sherlock_holmes
  - character/doctor_watson
  - archetype/detective
  - archetype/mentor
---
*by Conan Doyle*  ·  **Cluster:** [[clusters/detective-fiction-·-ratiocination-detection-·-crime-and-punishment|Detective Fiction · Ratiocination Detection · Crime and Punishment]]

> [!abstract] 
> A short comic Holmes vignette, written for an Edinburgh University charity magazine, in which Holmes deduces from Watson's demeanor and a single letter that Watson has been asked to contribute to a university bazaar in aid of the cricket field. The piece is a playful, self-aware parody of the familiar Baker Street breakfast-table deduction scene, followed in this volume by the companion miniature tale 'How Watson Learnt the Trick.'

## Connections

**Forms & Genres:** [[forms/short-story|Short Story]], [[forms/detective-fiction|Detective Fiction]], [[forms/parody|Parody]]
**Themes & Concepts:** [[concepts/ratiocination-detection|Ratiocination Detection]], [[concepts/appearance-vs-reality|Appearance vs Reality]]
**Motifs:** [[motifs/letters-and-writing|Letters and Writing]]
**Settings:** [[settings/london|London]]
**Characters:** Sherlock Holmes, Doctor Watson
**Archetypes:** Detective, Mentor
