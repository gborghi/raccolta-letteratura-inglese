...
function populateVirtualPageHtmlAst(
  ...
  for (const ve of virtualEntries) {
    ...
    try {
      ...
      // NOTE: do NOT do `ve.tree.children = htmlAst.children` here — that mutates the
      // emit tree with the rendered body, so FolderContent/TagContent render their
      // listing twice (once as `content`, once as `pageBody`). Keep htmlAst on the
      // vfile for transclusion only.
